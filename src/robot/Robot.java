...
public class Robot {
	...
	public void move() {
		...
		if (other != null) {
			if (other.targetX == targetX && other.targetY == targetY) {// not
																		// allowed
																		// to
																		// move
				targetX = x;
	...
	public int getBarrelLR(int n) {
		...
		debug(((n == 0) ? "" : (n + "th")) + "closestBarrelLR = "
	...
	public int getBarrelFB(int n) {
		...
		debug(((n == 0) ? "" : (n + "th")) + "closestBarrelFB = "
				+ ((val == INFINITY) ? "INFINITY" : "" + val));
		return val;
	}
	...
	public static final int ACTION_MOVE = 1, ACTION_TURN_LEFT = 2,
			ACTION_TURN_RIGHT = 3, ACTION_TURN_AROUND = 4,
			ACTION_TAKE_FUEL = 5, ACTION_WAIT = 6, DIR_NORTH = 1, DIR_WEST = 2,
			DIR_SOUTH = 3, DIR_EAST = 4, INFINITY = Integer.MAX_VALUE;

	private static final int FUEL_IDLE = 3, FUEL_MOVE = 6, FUEL_TURN = 5,
			FUEL_SHIELD = 15;
	...
	public Robot(World world, int x, int y, String colour) {
		...
		targetFuel = fuel;
		try {
			robotImage1 = ImageIO.read(new File("assets/robot_" + colour
					+ "_1.png"));
			robotImage2 = ImageIO.read(new File("assets/robot_" + colour
					+ "_2.png"));
	...
	public void draw(Graphics2D g2d, double time) {
		...
		trans.translate(x, y);
		trans.rotate(angle);
		trans.translate(-robotImage1.getWidth() / 2d,
				-robotImage1.getHeight() / 2d);

		if (((int) (time * 10)) % 2 == 0 && currentAction < ACTION_TAKE_FUEL
				&& currentAction > 0) {
		...
		// draw fuel indicator
		Arc2D fuelArc = new Arc2D.Double(x - 10, y - 10, 20, 20, -90,
				360d * fuelCurrent / 100d, Arc2D.OPEN);
